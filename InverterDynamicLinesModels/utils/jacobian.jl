struct ModelJacobian{T <: InverterModel}
    J_func::Function
    J_Matrix::Matrix{Float64}
end

function get_jacobian_function(::Type{T}) where T <: InverterModel
...
function instantiate_jacobian(M::ModelOperatingPoint{T}) where T <: InverterModel
    jac = get_jacobian_function(T)
    jac_eval = (out, u0, params) -> jac(out, u0, params)
    param_eval = (out, params) -> jac(out, M.u0, params)
    n = length(M.u0)
    J = zeros(n, n)
    param_eval(J, M.parameters)
    return ModelJacobian{T}(jac_eval, J)
end
