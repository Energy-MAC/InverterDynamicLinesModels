function get_internal_model_vsm_alglines(::Nothing)
    # Model Parameters
    ...
    Zmag = rg.^2 + ω_sys.^2 * lg.^2

    model_rhs = [
        # Algebraic Line Equations
        #I = YV
        #Re[0 = I-YV]
        if_r - (1 / Zmag) * ( rg * (vg_from_r - vg_to_r) + ω_sys * lg * (vg_from_i - vg_to_i) ) + ω_sys * cg * vg_from_i
        #Im[0 = I-YV]
        if_i - (1 / Zmag) * ( rg * (vg_from_i - vg_to_i) - ω_sys * lg * (vg_from_r - vg_to_r) ) - ω_sys * cg * vg_from_r
        #Filter Equations
        #𝜕ef_d/𝜕t
...
function get_ode_system_vsm_alglines()
    model_lhs, model_rhs, states, _, params = get_internal_model_vsm_alglines(nothing)
    t = params[1]
    _eqs = model_lhs .~ model_rhs
    return MTK.ODESystem(_eqs, t, [states...], [params...][2:end])
end

function get_nonlinear_system_vsm_alglines()
